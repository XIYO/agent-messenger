...
// Upload a custom emoji. The filename decides the media type Discord is sent.
...
// → DiscordSticker[] { id, name, description?, tags, format_type?, ... }
...
image is read. Discord refuses an upload whose multipart part carries no media
type, so the filename is passed alongside the bytes and used to derive it.

Both endpoints need the Manage Expressions permission. `getServer(guildId)`
