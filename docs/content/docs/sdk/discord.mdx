...
with code `unsupported_asset` before any request is made. A leading `{` proves
only that the file is JSON — whether it is a usable Lottie is Discord's call,
and Lottie uploads need a `VERIFIED` or `PARTNERED` guild.
