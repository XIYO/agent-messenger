...
A sticker image must be a PNG, APNG or GIF at exactly 320x320, or a Lottie
JSON, and at most 512KB. Animated stickers are capped at 5 seconds. Lottie
stickers upload only on guilds with the `VERIFIED` or `PARTNERED` feature.
