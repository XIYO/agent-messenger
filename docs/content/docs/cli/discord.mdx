...
The media type is read from the file's own bytes, not its extension. A file
whose contents are not a PNG, GIF, JPEG, WebP or Lottie JSON is refused before
the request, because Discord answers a mismatched part with `Invalid Asset`.
...
A sticker image must be a PNG, APNG, GIF or Lottie JSON, at most 512KB.
