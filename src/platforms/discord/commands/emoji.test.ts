...
it('create: uploads the image with its content type and derives the name from the filename', async () => {
  ...
  expect(clientCreateEmojiSpy).toHaveBeenCalledWith('g1', 'potato_13', expect.any(Uint8Array), 'image/png', [])
...
it('create: rejects an image over 256KB without calling the API', async () => {
  const consoleSpy = mock((_msg: string) => {})
  console.log = consoleSpy
  const path = await writeTempImage('potato_13.png', pngBytes(128, 128, 256 * 1024))

  await expect(createAction('g1', path, { pretty: false })).rejects.toThrow('process.exit(1)')

  expect(clientCreateEmojiSpy).not.toHaveBeenCalled()
  expect(consoleSpy.mock.calls[0][0]).toContain('256KB')
})
